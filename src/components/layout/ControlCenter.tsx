...
export function ControlCenter({ prices, isSyncing, onSync }: { prices: CryptoPrice[], isSyncing: boolean, onSync: () => void }) {
  const { balance, portfolio, learningXP, portfolioAudit, properties, achievements } = useVantageStore();
  ...
  const totalNetWorth = (balance || 0) + portfolioMarketValue + propertiesValue;
  ...
  return (
    <div className="pt-20 sm:pt-24 px-4 sm:px-8 max-w-[1400px] mx-auto space-y-6 pb-24 no-scrollbar">
      ...
      <div className="flex justify-between items-center py-5 border-b border-[#2B2F36]">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-semibold uppercase tracking-widest text-[#848E9C]">Сатҳи_ваколати_система_01</span>
          </div>
          <h1 className="text-2xl sm:text-4xl font-extrabold text-[#EAECEF] tracking-tight">ПАНЕЛИ ДИДБОН</h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard label="Арзиши Умумии Сармоя" value={`$${totalNetWorth.toLocaleString()}`} subValue="Маблағҳои Нақд + Портфел" trend={14.2} icon={Layers} />
        <StatCard label="Маблағҳои Озоди Нақд"  value={`$${(balance || 0).toLocaleString()}`} subValue="Омода барои сармоягузорӣ" icon={Activity} />
        <StatCard label="Сатҳи Академия" value={`Сатҳи ${Math.floor(learningXP / 100) + 1}`} subValue={`${100 - (learningXP % 100)} XP то сатҳи навбатӣ`} icon={Brain} />
      </div>
            ...
            <MonolithCard className="p-5 sm:p-8 h-full">
                <div className="flex items-center justify-between mb-6">
                   <div className="flex items-center gap-3">
                      ...
                      <h3 className="text-base sm:text-xl font-bold uppercase tracking-tight text-[#EAECEF]">ТАҲЛИЛИ НЕЙРОНИИ ПОРТФЕЛ</h3>
                   </div>
                ...
                <div className="space-y-6">
                   <p className="text-sm text-[#848E9C] leading-relaxed border-l-2 border-[#F0B90B]/30 pl-4 py-1">
                     {portfolioAudit || 'Таҳлили нейронии портфели шумо пас аз аввалин сармоягузорӣ дар ин ҷо пайдо мешавад. Интеллекти сунъӣ хавфҳои шуморо таҳлил карда, тавсияҳои тактикӣ медиҳад.'}
                   </p>
                   <div className="grid grid-cols-2 gap-4 pt-2">
                      {/* Market Flux */}
                      <div className="bg-[#2B2F36] rounded-lg p-4">
                         <div className="text-[10px] font-semibold uppercase tracking-widest text-[#848E9C] mb-2">Тағйирёбии_Бозор</div>
                      ...
                      {/* Portfolio Status */}
                      <div className="bg-[#2B2F36] rounded-lg p-4">
                         <div className="text-[10px] font-semibold uppercase tracking-widest text-[#848E9C] mb-2">Ҳолати_Портфел</div>
                         <div className={`text-2xl sm:text-3xl font-bold font-mono ${portfolio.length > 0 ? 'text-[#F0B90B]' : 'text-[#848E9C]'}`}>
                           {portfolio.length > 0 ? 'ФАЪОЛ' : 'ҒАЙРИФАЪОЛ'}
                         </div>
                      </div>
                   </div>
                </div>
         ...
         <div className="lg:col-span-4">
            <MonolithCard className="p-5 sm:p-8 h-full">
               <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-base font-bold uppercase tracking-tight text-[#EAECEF]">НИШОНДИҲАНДАИ ТАЪСИР</h3>
               </div>
               <div className="space-y-6">
                 {[
                   { label: "Таъсири_Институтсионалӣ", val: 88 },
                   { label: "Самаранокии_Захираҳо",    val: 94 }
                 ].map(s => (
            ...
            <MonolithCard className="p-5 sm:p-8 h-full flex flex-col justify-between">
               ...
               <div className="mt-6 pt-4 border-t border-[#2B2F36] flex items-center gap-2.5 text-[#474D57]">
                  ...
                  <span className="text-[10px] font-semibold uppercase tracking-wider">
                     ТАҲЛИЛИ МАКРОЭКОНОМИКИИ СОҲИБИХТИЁР ОНЛАЙН АСТ
                  </span>
               </div>
            </MonolithCard>
         </div>
         ...
         <div className="lg:col-span-7">
            <MonolithCard className="p-5 sm:p-8 h-full">
               <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="text-base sm:text-xl font-bold uppercase tracking-tight text-[#EAECEF]">
                     МАТРИТСАИ ДАСТОВАРДҲОИ СОҲИБИХТИЁР
                  </h3>
               </div>
